...
def fillHists(uid,runid,subid):
    ...
    #
    fn = runpath+"/td00"+str(runid)+"_f00"+str(subid)+".root"
    ...
    for i, v in enumerate(trawv):
        ...
        #dans trace: 3 champs (num du, channel, longueur trace)
    ...
    fnpz = "td00"+str(runid)+"_f00"+str(subid)+"_DU"+str(uid)+".npz"
    if len(sig)>0:
      np.savez(fnpz, sig=sig, time = gpstime, freq = freqx, bat = battery, mfft = mfft/nev, trace = trace)
    else:
...
# ### Loop on units/runs
for uid in [83]:
#for uid in [83, 70, 49, 58, 59, 60, 144, 84]:
    fn = "td00"+str(runid)+"_f00"+str(subid)+"_DU"+str(uid)+".npz"
    if os.path.isfile(fn) == False:
